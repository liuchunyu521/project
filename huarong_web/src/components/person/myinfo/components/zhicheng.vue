...
<script>
  ...
  import subDialog from './subdialog';
	...
	export default {
		...
		data() {
			return {
        infoSetCode: 'hi_psndoc_title', //接口关键字
        dialogVisible: false, //新增编辑弹窗的显示隐藏字段
        dialogData: {}, //弹窗的数据
        postSaveData: {
          "transType": "savePsnInfo",
          "infoSetCode": 'hi_psndoc_title'
        }, //新增编辑删除
        initData:{
        }
			}
    ...
    computed:{
      theadData(){
        return this.$store.state.Information['zhichenTheadData']
      },
      formConfig(){
        return this.$store.state.Information['zhichenFormConfig']
      }
    },
		components: {
			yInput, //表单元素
...
//         debugger;
      ...
      validateForm: function( data ){
        ...
        if( _sdate && _sdate != "" && _edate && _edate != "" ){
          if( _edate < _sdate ){
            this.$message.error('结束日期不能早于评定日期');
            return false;
          };
        };
        return true;
