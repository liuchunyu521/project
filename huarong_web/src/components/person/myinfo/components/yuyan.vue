...
<script>
  ...
  import subDialog from './subdialog';
	...
	export default {
		...
		data() {
			return {
        infoSetCode: 'hi_psndoc_langability', //接口关键字
        dialogVisible: false, //新增编辑弹窗的显示隐藏字段
        dialogData: {}, //弹窗的数据
        postSaveData: {
          "transType": "savePsnInfo",
          "infoSetCode": 'hi_psndoc_langability'
        }, //新增编辑删除
        initData: {
        }
			}
    ...
    computed:{
      theadData(){
        return this.$store.state.Information['yuyanTheadData']
      },
      formConfig(){
        return this.$store.state.Information['yuyanFormConfig']
      }
    },
		components: {
			yInput, //表单元素
