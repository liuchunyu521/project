...
<script>
  ...
  import subDialog from './subdialog';
	...
	export default {
		...
		data() {
			return {
        infoSetCode: 'hi_psndoc_abroad', //接口关键字
        dialogVisible: false, //新增编辑弹窗的显示隐藏字段
        dialogData: {}, //弹窗的数据
        postSaveData: {
          "transType": "savePsnInfo",
          "infoSetCode": 'hi_psndoc_abroad'
        }, //新增编辑删除
        initData: { }
			}
		},
    computed:{
      theadData(){
        return this.$store.state.Information['chuguoTheadData']
      },
      formConfig(){
        return this.$store.state.Information['chuguoFormConfig']
      }
    },
		components: {
			yInput, //表单元素
      ...
      validateForm: function( data ){
        var _sdate = ( data.abroaddate ? data.abroaddate : this.dialogData.abroaddate );
        var _edate = ( data.abroadreturn ? data.abroadreturn : this.dialogData.abroadreturn );

        if( _sdate && _sdate != "" && _edate && _edate != "" ){
          if( _edate < _sdate ){
            this.$message.error('回国日期不能早于出国日期');
            return false;
          };
        };
        return true;
