<template>
    <ifbp-page>
        <!-- 节点title -->
        <ifbp-breadcrumb name="转正页面设置"></ifbp-breadcrumb>
        <!-- 按钮区域 -->
        <ifbp-button-area>
            ...
            <div class="fr">
                <span class="search-label">&nbsp;试用期类型&nbsp;:&nbsp;</span>
                <el-select v-model="probation_type" placeholder="请选择" :disabled="searchDisabled"
                           @change="handleSearchChange" style="width:130px;">
                    <el-option key="1" label="入职试用" value="1"/>
                    <el-option key="2" label="转岗试用" value="2"/>
                </el-select>

            </div>

            <div class="fr">
                <span class="search-label">人力资源组织&nbsp;:&nbsp;</span>

                <el-ref ref="searchOrg"
                        v-model="searchEntity.pk_org"
                        field="pk_org"
                        :template-value="searchEntity"
                        :not-leaf-selectable="true"
                        ref-code="adminOrgGroupPowerRef" :disabled="searchDisabled"
...
//          //替换列表显示为switch开关控件
        ...
        methods: {
            //初始化
            init(){
                var url = '/hrtrn/basic/regitem/init';
                this.$http({
                    url: url,
                    method: "get",
                    dataType: "json"
                    ...
                    .then((res) => {
                        if (res.data.status === true) {
                            ...
                            this.$refs.searchOrg.setPK(res.data.default_org);

                            this.$refs.ref_reg_itemset.setData("hi_reg_itemset_t", JSON.parse(JSON.stringify(res.data.data)));
                        } else {
                            this.$message({
            ...
            save(){
                let data = this.$refs.ref_reg_itemset.getData("hi_reg_itemset_t");
                let pk_org = this.searchEntity.pk_org;
                var url = '/hrtrn/basic/regitem/save?pk_org=' + pk_org + '&probationType=' + this.probation_type;
                this.$http({
                    url: url,
                    method: "post",
                    data: data
            ...
            handleSearchChange(aa){
                let pk_org = this.searchEntity.pk_org;
                var url = '/hrtrn/basic/regitem/search?pk_org=' + pk_org + '&probationType=' + this.probation_type;
                this.$http({
                    url: url,
                    method: "get",
                    dataType: "json"
