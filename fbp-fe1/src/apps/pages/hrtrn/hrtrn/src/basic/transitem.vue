<template>
    <ifbp-page>
        <!-- 节点title -->
        <ifbp-breadcrumb name="调动页面设置"></ifbp-breadcrumb>
        <!-- 按钮区域 -->
        <ifbp-button-area>
            ...
            <div class="fr">
                <span class="search-label">调动业务类型&nbsp;:&nbsp; </span>
...
//          //替换列表显示为switch开关控件
        ...
        methods: {
            ...
            init(){
                ...
                var url = '/hrtrn/basic/transitem/init';
                this.$http({
                    url: url,
                    method: "get",
                    dataType: "json"
...
//              this.options = res.data.orgs;
//                this.selectedOptions = res.data.default_org;
...
//                let data = this.$refs.ref_reg_itemset.getData("hi_reg_itemset_t");
                if (data != null && data !== undefined && data.length > 0) {
                    for (var i = 0; i < data.length; i++) {
                        ...
                        } else if (itemkey === "pk_org" || itemkey === "pk_dept" ) {

                        /*} else if (itemkey === "pk_org" || itemkey === "pk_dept" || itemkey === "pk_post") {*/
                            data[i].cl_isshow_disabled = true;
                            data[i].cl_isedit_disabled = true;
                            data[i].cl_ismustnotnull_disabled = true;
                            data[i].cl_isdefault_disabled = false;
            ...
            save(){
                ...
                let data = this.$refs.ref_reg_itemset.getData("hi_reg_itemset_t");
                var url = '/hrtrn/basic/transitem/save?pkstType=' + this.pk_sttype;
                this.$http({
                    url: url,
                    method: "post",
                    data: data
            ...
            handleSearchChange(aa){
                var url = '/hrtrn/basic/transitem/search?pkstType=' + this.pk_sttype;
                this.$http({
                    url: url,
                    method: "get",
                    dataType: "json"
