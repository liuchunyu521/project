<template>
    <ifbp-page>
        <ifbp-breadcrumb v-if="isNew" name="职务图谱"></ifbp-breadcrumb>
        ...
        <div id="indexTab">
            ...
            <!--<div class="abso zhiwu" v-bind:style="{left:everyWidth*2+'px'}">职务类别</div>-->
            <div class="abso zhiji" style="width: 100px">职级类别</div>
            <div class="abso zhijiLevel" style="width: 100px;left:100px">职级</div>
            <div class="abso zhiwu" style="left:200px">职务类别</div>

        </div>
        <div id="mydiv" style="position:absolute;display:none;border:1px solid silver;background:silver;">
        </div>
        <!--</ifbp-main-area>-->
    </ifbp-page>
</template>
...
//                axios.get('/hrjf/jobChart/job/getAll')//接口3 中
                axios.get('/hrjf/jobOVP/jobChart/jobType'),//接口1 上
                axios.get('/hrjf/jobOVP/jobChart/levelTypeAndLevel'), //接口2 左
                axios.get('/hrjf/jobOVP/jobChart/job')//接口3 中
...
//1.标识X和Y轴对象，获取X轴深度
                    vm.xCount = vm.markAxis(vm.zhiwu,vm.xAxis,vm.count,"pk_jobtype");
...
// 5.遍历内容
                    vm.ergodic();
//得到平均宽度everyWidth
                    var indexTabWidth = document.getElementById("indexTab").offsetWidth;//外层宽
...
//                  6.渲染x轴
                    ...
                    document.getElementsByClassName("zhiji")[0].style.lineHeight = vm.deepl+'px';//职级类别
                    document.getElementsByClassName("zhijiLevel")[0].style.lineHeight =  vm.deepl+"px";//职级
                    document.getElementsByClassName("zhiwu")[0].style.width =
...
// x轴
            DOMTag:function(jobtypename,left,top, width,height){//添加div
                ...
                document.getElementById("indexTab").appendChild(Div);
                return Div;
            },
            digui:function(node,left,level){ //递归
                ...
                node.forEach(function(item,index,arr){//遍历职务数组
                    ...
                    var dom = vm.DOMTag(item.jobtypename, left , top , width);//绘制DOM
...
// y轴
            DOMTagY:function(name,leftY,topY, herightY){//添加div  @mouseenter="enter" @mouseleave="leave"
                var vm = this;
                var Div = document.createElement("div");
                Div.innerHTML   = name;//div内容
                Div.style.width =  '100px';//'160px';vm.everyWidth
            ...
            diguiY:function(node,topY){//node=zhiji
                ...
                node.forEach(function(item,index,arr){//遍历职务数组
                    if(item.hasOwnProperty("leveltypename")){//一层渲染
                        ...
                        var herightYn = 48;
                        var dom = vm.DOMTagY(item.levelname, 100 ,topYn, herightYn);//160
                    }
...
//                for(var i=0;i<arr.length;i++){
                        ...
                        if(Object.prototype.toString.call(item.pk_level) == "[object Array]" ){//判断是否为数组 如果是则循环展示Object
                            item.pk_level.forEach(function(items,index){
                                if(items != ""){
                                    ...
                                    var xAxisson =  vm.xAxis[item.pk_jobtype];//X轴
                                    ...
                                    var xAxisson =  vm.xAxis[item.pk_jobtype];//职务
                                    if(xAxisson == 'undefined'|| xAxisson == undefined){
                                        return
                                    }
                                    var X = xAxisson.pos;
                                    ...
                                    if (content[X + '_' + Y]){ //如果当前格子数组已创建
                                        ...
                                        if(content[X + '_' + (Y-1)] != undefined){//如果上有和它一样数据 遍历寻找一样 并拼接到其下方
                                            ...
                                            for(var i=0;i<content[X + '_' + (Y-1)].length;i++){
                                                if(content[X + '_' + (Y-1)][i] && content[X + '_' + (Y-1)][i]._div.innerHTML == item.jobname){//当找到与上一行一样的内容
                                                    ...
                                                    for(var j=0;j<=content[X + '_' + Y].length-1;j++){
                                                        ...
                                                        if(content[X + '_' + Y][j] != undefined &&  content[X + '_' + Y][j]._div.countLeft == sameCl){
                                                            ...
                                                            if(content[X + '_' + (Y+1)] && content[X + '_' + (Y+1)][j] && content[X + '_' + (Y+1)][j]._div.innerHTML == content[X + '_' + Y][j]._div.innerHTML){
                                                                ...
                                                                var up = vm.Up(preItem,item.jobname,content,X,Y);
                                        ...
                                        //1.衔接合并与上一样的DOM
                                        var upT = vm.UpT(preItem,item.jobname,content,X,Y);


                                        if(upT != true){//2.衔接合并与下一样的DOM
                                            var Down = vm.Down(preItemA,item.jobname,content,X,Y);
                                        }
                                        ...
                                        if(upT == true){
                                            ...
                                            for(var i=0;i<r;i++){//之前有空档
                                                if(content[X + '_' + Y][i] == null){//之前有空档 arrZ设置为0
                                                    ...
                                                    vm.createDom(content,item.jobname,L,i,T,X,Y);
                                                    arrZ = 0;
                                                }
                                            }
                                            if(arrZ == 1){//没空档

                                                vm.createDom(content,item.jobname,L,r+1,T,X,Y);
                                            }
                                        ...
                                        if(up != true && Down != true ){
                                            vm.createDom(content,item.jobname,L,0,T,X,Y);
                                        }
            ...
            createDom:function(content,jobname,L,cL,T,X,Y){//内容对象,名字,pos左边距,dom左边内距
                ...
                Div.style.left = ((L+Div.countLeft) * vm.everyWidth + 200) +'px';//320
...
//                        var top = top1+j*40;
            ...
            ergodic:function(){//遍历内容 得到重合对象
                ...
                vm.neirong.forEach(function(item,index,arr){//遍历内容
                    ...
                    var xItem = vm.xAxis[item.pk_jobtype];//X轴对象
