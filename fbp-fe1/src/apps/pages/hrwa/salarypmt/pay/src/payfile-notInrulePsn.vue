    ...
    methods: {
		checkchange(){
        		if(this.checkedall==true){
        			this.$refs.ref_delbyrule.getTableComp().selectAllRow();
        			this.$emit('setIsAllPageSelect', true);
        			
        		}else if(this.checkedall==false){
        			this.$refs.ref_delbyrule.getTableComp().unSelectAllRow();
	 ...
	 initPromise(request) {
      ...
      request(n,s) {
        ...
        var url =
          "/hrwa/salarymgt/payData/payFile/qryIllegalData";
        this.$http({
          url: url,
          method: "post",
          data : {
          ...
          .then(res => {
            if (res.data.status === true) {
              ...
              this.initPromise(this.$refs.ref_delbyrule.setData(
                "payfile_t",
