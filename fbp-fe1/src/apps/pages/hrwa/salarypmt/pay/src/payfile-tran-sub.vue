...
<template>
    <div>
      ...
      <ifbp-template ref="ref_alter_out"
                     tpl-id="alternewoff"
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      return {
        subPsnTempId: "0001AA1000000001FHER",
        subPsnData: {
        },
        currentPage:1,
        pageSize :1,
    ...
    methods: {
		checkchange(){
        		if(this.checkedall==true){
        			this.$refs.ref_alter_out.getTableComp().selectAllRow();
        			this.$emit('setIsAllPageSelect', true);
        		}else if(this.checkedall==false){
        			this.$refs.ref_alter_out.getTableComp().unSelectAllRow();
    ...
    setCheckedallValue(v){
	 ...
	 initPromise(request) {
        ...
        this.$http({
          ...
          .then(res => {
            if (res.data.status === true) {
              ...
              this.$emit("setTabLabel","离职人员("+this.totalElements+")");
                this.$emit("actionDisable",this.totalElements==0);
              this.initPromise(this.$refs.ref_alter_out.setData(
                "hi_psnjob_t",
      ...
      operationButtonClick() {
        var tableSelections = this.$refs.ref_alter_out.comp.$refs.ref_alter_out.getSelection();
        var delDatas = [];
