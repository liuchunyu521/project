...
<!--   :pk-temp="subPsnTempId"-->
      <ifbp-template ref="ref_alter_modify"
                     tpl-id="altertransfer"
...
<script>
  ...
  export default {
    ...
    data() {
      ...
      return {
        subPsnTempId: "0001AA1000000001FHEU",
        subPsnData: {
        },
        currentPage:1,
        pageSize :1,
    ...
    methods: {
		checkchange(){
        		if(this.checkedall==true){
        			this.$refs.ref_alter_modify.getTableComp().selectAllRow();
        			this.$emit('setIsAllPageSelect', true);
        		}else if(this.checkedall==false){
        			this.$refs.ref_alter_modify.getTableComp().unSelectAllRow();
	 ...
	 initPromise(request) {
        ...
        this.$http({
          ...
          .then(res => {
            if (res.data.status === true) {
              ...
              this.$emit("setTabLabel","调配人员("+this.totalElements+")");
                this.$emit("actionDisable",this.totalElements==0);
              this.initPromise(this.$refs.ref_alter_modify.setData(
                "hi_psnjob_t",
      ...
      operationButtonClick() {
        var tableSelections = this.$refs.ref_alter_modify.comp.$refs.ref_alter_modify.getSelection();
        var delDatas = [];
