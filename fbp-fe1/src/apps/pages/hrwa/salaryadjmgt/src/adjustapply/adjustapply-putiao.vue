...
//                            this.adjust_direction_sec_ismust = true;
                ...
                ruleResetFun: function ($node) {
                    var $pk_org = $node.find("[prop='pk_org']");
                    $pk_org.attr('hide', 'false');
                    $pk_org.attr(':required', 'true');
...
//                        + '</el-table-column>'
                        ...
                        + '<el-table-column  sortable render-type="money" separator="," :multiply="false" symbol="￥" align="left" header-align="left" prop="money_adjust" label="金额" width="100" format="">'
//                        + '<template scope="scope">'
...
//                        this.moneyeditable = val.target.checked;
        ...
        methods: {
            ...
            next() {
                this.$refs.ref_ruleitem_c.validate((valid) => {
                    if (valid) {
                        ...
                        this.$http({
                            url: "/hrwa/psndocwadoc/queryWadoc4AdjustApply",
                            method: "post",
            ...
            submit(){
                ...
                this.$http({
                    url: "/hrwa/psndocwadoc/confirmAdjustApplyWadoc",
                    method: "post",
                    ...
                    .then((res) => {
                        if (res.data.status === true) {
                            ...
                            sessionStorage.setItem("applySubData",JSON.stringify(res.data.data));
                            sessionStorage.setItem("pk_org",this.adjustData.base.pk_org);
                            sessionStorage.setItem("hrwaadjust-backnum",2);
                            sessionStorage.setItem("pk_changecause",this.adjustData.base.pk_changecause);
                            sessionStorage.setItem("pk_changecause_ref",JSON.stringify(this.adjustData.base.beanMap.pk_changecause_ref));
                        } else {
                            this.$message({
                                message: res.data.msg,
                                type: "error"
