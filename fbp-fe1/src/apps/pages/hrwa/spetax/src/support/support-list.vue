...
<template>
    <ifbp-page>
        ...
        <ifbp-main-area type="list">
            ...
            <div>
                <!-- UI模板组件 -->
                <ifbp-template  ref="ref_support_l"
                                tpl-id="support-list"
                                :tpl-data="tableData"
...
<script>
    ...
    export default {
        ...
        data() {
        	const supportDetailThis = this;
            return {  
            	 // 查询模板编码
            searchCode: 'support-query',
            dialogTitle: '过滤',
            pk_org: '',
            selectedTreeNode: [],
            showEnd: false,
            ...
            chlideduTableResetFun: function ($node) {
                ...
                billcode.html(
                    ...
                    '<a href="javascript:void(0);" @click="supportDetailClick(scope)">{{scope.row.billcode}}</a>' +
                    '</template>'
                );
            ...
            tableFormMethods: {
                // 点击行超链接事件
                supportDetailClick: function (scope) {
                    supportDetailThis.detailPage(scope.row.pk_support_h);
                },
            },
            // 分页组件传入参数
            totalElements: 0,
...
//	    },
        ...
        methods: {
		        ...
		        // table行的编辑操作
		        tbleEditClick(scope) {
		            this.detailPage(scope.row.pk_support_h);
		        },
		        // table行的删除操作
		        tbleDeleteClick(scope) {
		            this.$confirm('确认删除该数据？删除后无法恢复。', '提示', {
		                ...
		                this.delbill(scope.row.pk_support_h)
		            }).catch(() => {
		                // 这里加取消逻辑
		            });
		        },
		        ...
		        tableCommitClick(scope) {
		            ...
		            this.$http({
		                ...
		                method: "post",
		                data: {
		                    pk_support_h: scope.row.pk_support_h
		                }
		            }).then(res => {
		                if (res.data.status === true) {
		                    this.$message({ message: res.data.msg, type: "success" });
		        ...
		        detailPage: function (pk) {
		            this.$router.push("/hrwa/spetax/support/detail/" + pk);
		        },
		        request(n, s) {
		            var url = "/hrwa/spetax/support/pageList";
		            let param = {
		                pn: n,
		                ps: s,
		                pk_org: this.pk_org,
		            ...
		            this.$http({
		                ...
		                .then((res) => {
		                    if (res.data.data && res.data.data.content != null) {
		                        this.$refs.ref_support_l.setTableData(res.data.data.content);
		                        this.totalElements = res.data.data.totalElements;
		                        this.size = res.data.data.size;
		                    } else {
		                        this.$refs.ref_support_l.setTableData(null);
		                    }
		        ...
		        addbill() {
		            this.$router.push("/hrwa/spetax/support/detail/");
		        },
