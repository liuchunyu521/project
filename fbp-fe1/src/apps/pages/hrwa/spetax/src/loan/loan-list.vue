...
<template>
     <ifbp-page>
        ...
        <ifbp-main-area type="list">
            ...
            <div>
                <!-- UI模板组件 -->
                <ifbp-template  ref="ref_loan_l"
                                tpl-id="loan-list"
                                :tpl-data="tableData"
                                :table-show-menu="false"
                                :methods="tableFormMethods"
                                :tplResetFun="loanTableResetFun"
...
export default {
    ...
    data() {
        const loanDetailThis = this;
        return {
            // 查询模板编码
            searchCode: 'loan-query',
            dialogTitle: '过滤',
            pk_org: '',
            selectedTreeNode: [],
            showEnd: false,
            ...
            tableData: {},
            loanTableResetFun: function ($node) {
                debugger
                // 申请单编码
                let billcode = $node.find("[prop='billcode']");
                billcode.html(
                    '<template scope="scope">' +
                    '<a href="javascript:void(0);" @click="loanDetailClick(scope)">{{scope.row.billcode}}</a>' +
                    '</template>'
                );
            ...
            tableFormMethods: {
                // 点击行超链接事件
                loanDetailClick: function (scope) {
                    loanDetailThis.detailPage(scope.row.pk_loan_h);
                },
            },
            // 分页组件传入参数
            totalElements: 0,
...
//  },
    ...
    methods: {
        ...
        // table行的编辑操作
        tbleEditClick(scope) {
            this.detailPage(scope.row.pk_loan_h);
        },
        // table行的删除操作
        tbleDeleteClick(scope) {
            this.$confirm('确认删除该数据？删除后无法恢复。', '提示', {
                ...
                this.delbill(scope.row.pk_loan_h)
            }).catch(() => {
                // 这里加取消逻辑
            });
        },
        ...
        detailPage: function (pk) {
            this.$router.push("/hrwa/spetax/loan/detail/" + pk);
        },
        request(n, s) {
            debugger
            var url = "/hrwa/spetax/loan/pageList";
            let param = {
                pn: n,
                ps: s,
                pk_org: this.pk_org,
            ...
            this.$http({
                ...
                .then((res) => {
                    if (res.data.data && res.data.data.content != null) {
                        this.$refs.ref_loan_l.setTableData(res.data.data.content);
                        this.totalElements = res.data.data.totalElements;
                        this.size = res.data.data.size;
                    } else {
                        this.$refs.ref_loan_l.setTableData(null);
                    }
        ...
        addbill() {
            this.$router.push("/hrwa/spetax/loan/detail/");
        },
