...
<template>
     <ifbp-page>
        ...
        <ifbp-main-area type="list">
            ...
            <div>
                <!-- UI模板组件 -->
                <ifbp-template  ref="ref_contedu_l"
                                tpl-id="contedu-list"
                                :tpl-data="tableData"
                                :table-show-menu="false"
                                :editable="false"
                                :methods="tableFormMethods"
                                :tplResetFun="continueduTableResetFun"
...
<script>
    ...
    export default {
        ...
        data() {
	        const conteduDetailThis = this;

            return {  
        	        // 查询模板编码
		            searchCode: 'contedu-query',
		            dialogTitle: '过滤',
		            pk_org: '',
		            selectedTreeNode: [],
		            showEnd: false,
		            ...
		            tableData: {},
		            continueduTableResetFun: function ($node) {
		                // 申请单编码
		                let billcode = $node.find("[prop='billcode']");
		                billcode.html(
		                    '<template scope="scope">' +
		                    '<a href="javascript:void(0);" @click="conteduDetailClick(scope)">{{scope.row.billcode}}</a>' +
		                    '</template>'
		                );
		            ...
		            tableFormMethods: {
		                // 点击行超链接事件
		                conteduDetailClick: function (scope) {
		                    conteduDetailThis.detailPage(scope.row.pk_contedu_h);
		                },
		            },
		            // 分页组件传入参数
		            totalElements: 0,
...
//  	},
	   created() {
        ...
        methods: {
		        ...
		        // table行的编辑操作
		        tbleEditClick(scope) {
		            this.detailPage(scope.row.pk_contedu_h);
		        },
		        // table行的删除操作
		        tbleDeleteClick(scope) {
		            this.$confirm('确认删除该数据？删除后无法恢复。', '提示', {
		                ...
		                this.delbill(scope.row.pk_contedu_h)
		            }).catch(() => {
		                // 这里加取消逻辑
		            });
		        },
		        ...
		        tableCommitClick(scope) {
		            ...
		            this.$http({
		                ...
		                method: "post",
		                data: {
		                    pk_contedu_h: scope.row.pk_contedu_h
		                }
		            }).then(res => {
		                if (res.data.status === true) {
		                    this.$message({ message: res.data.msg, type: "success" });
		        ...
		        detailPage: function (pk) {
		            this.$router.push("/hrwa/spetax/contedu/detail/" + pk);
		        },
		        request(n, s) {
		            var url = "/hrwa/spetax/contedu/pageList";
		            let param = {
		                pn: n,
		                ps: s,
		                pk_org: this.pk_org,
		            ...
		            this.$http({
		                ...
		                .then((res) => {
		                    if (res.data.data && res.data.data.content != null) {
		                        this.$refs.ref_contedu_l.setTableData(res.data.data.content);
		                        this.totalElements = res.data.data.totalElements;
		                        this.size = res.data.data.size;
		                    } else {
		                        this.$refs.ref_contedu_l.setTableData(null);
		                    }
		        ...
		        addbill() {
		            this.$router.push("/hrwa/spetax/contedu/detail");
		        },
