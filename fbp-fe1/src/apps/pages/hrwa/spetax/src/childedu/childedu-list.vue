...
<template>
     <ifbp-page>
        ...
        <ifbp-main-area type="list">
            ...
            <div>
                <!-- UI模板组件 -->
                <ifbp-template  ref="ref_childedu_l"
                                tpl-id="childedu-list"
                                :tpl-data="tableData"
...
export default {
    ...
    data() {
        const childeduDetailThis = this;
        return {
            // 查询模板编码
            searchCode: 'childedu-query',
            dialogTitle: '过滤',
            pk_org: '',
            selectedTreeNode: [],
            showEnd: false,
            ...
            chlideduTableResetFun: function ($node) {
                ...
                billcode.html(
                    ...
                    '<a href="javascript:void(0);" @click="childeduDetailClick(scope)">{{scope.row.billcode}}</a>' +
                    '</template>'
                );
            ...
            tableFormMethods: {
                // 点击行超链接事件
                childeduDetailClick: function (scope) {
                    childeduDetailThis.detailPage(scope.row.pk_childedu_h);
                },
            },
            // 分页组件传入参数
            totalElements: 0,
...
//  },
    ...
    methods: {
        ...
        // table行的编辑操作
        tbleEditClick(scope) {
            this.detailPage(scope.row.pk_childedu_h);
        },
        // table行的删除操作
        tbleDeleteClick(scope) {
            this.$confirm('确认删除该数据？删除后无法恢复。', '提示', {
                ...
                this.delbill(scope.row.pk_childedu_h)
            }).catch(() => {
                // 这里加取消逻辑
            });
        },
        ...
        tableCommitClick(scope) {
            ...
            this.$http({
                ...
                method: "post",
                data: {
                    pk_childedu_h: scope.row.pk_childedu_h
                }
            }).then(res => {
                if (res.data.status === true) {
                    this.$message({ message: res.data.msg, type: "success" });
        ...
        callbackBill(scope) {
            ...
            this.$http({
                ...
                data: {
                    ...
                    param: param,
                    billType: "TAX1",
                    billId: scope.row.pk_childedu_h,
                    agentuserId: "",
                    pk_checkflow: ""
                }
            }).then(res => {
        ...
        detailPage: function (pk) {
            this.$router.push("/hrwa/spetax/childedu/detail/" + pk);
        },
        request(n, s) {
            var url = "/hrwa/spetax/childedu/pageList";
            let param = {
                pn: n,
                ps: s,
                pk_org: this.pk_org,
            ...
            this.$http({
                ...
                .then((res) => {
                    if (res.data.data && res.data.data.content != null) {
                        this.$refs.ref_childedu_l.setTableData(res.data.data.content);
                        this.totalElements = res.data.data.totalElements;
                        this.size = res.data.data.size;
                    } else {
                        this.$refs.ref_childedu_l.setTableData(null);
                    }
        ...
        addbill() {
            this.$router.push("/hrwa/spetax/childedu/detail/");
        },
