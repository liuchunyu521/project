...
<template>
  <ifbp-page>
    ...
    <ifbp-footer v-show="ifbpFooterShow">
      <el-button type="primary" v-if="partBtnShow" @click="employeeGoToPage(partTimeURL)">结束兼职</el-button>
    ...
    <ifbp-main-area type="detail">
      <ifbp-panel-group :navbar="true">
        <!-- 人员详情部分 -->
        <ifbp-panel id="employeePsndocPanel" title="基本信息" :icons="employeeDetail.psndoc.icons">
          <psndoc-main-avatar :funcode="funcode" :requestPrefix="requestPrefix"
                              :pk_psndoc="pk_psndoc" :psndocData="employeeData"
                              :templateEdit="employeeDetail.psndoc.edit"
                              :mode="3"
                              @dataChange="psndocDataChange">
          </psndoc-main-avatar>
          <psndoc-main  :ref="employeeDetail.psndoc.ref" 
                        :pk_psndoc="pk_psndoc"
                        tpl_id="hrhi_employee_detail_psndoc"
                        :funcode="funcode" :requestPrefix="requestPrefix"
                        :codeEditable="codeEditable"
                        :psndocData="employeeData"
                        :psndocMainEdit="employeeDetail.psndoc.edit"
                        :psndocMainBtn="employeeDetail.psndoc.edit"
        ...
        <ifbp-panel id="psnjobPanel" title="任职记录" >
          <psndoc-subset-psnjob :ref="employeeDetail.psnjob.ref" :pk_psndoc="pk_psndoc"
                                tpl_id="hrhi_employee_detail_psnjob"
                                :funcode="funcode"
...
export default {
  ...
  computed:{
    regApplyURL:function () {
      return '/hrtrn/trial/jldetail/' + this.pk_psndoc + "?funcode=60090regrds";
    },
    stApplyURL:function () {
      return '/hrtrn/transfer/jldetail/' + this.pk_psndoc + "?funcode=60090transrds";
    },
    partTimeURL:function () {
      return '/hrhi/employee/endpart/' + this.pk_psndoc + "?funcode=60070employee";
    }
  },
    ...
    ctrtIcons.icons = [
      {
        ...
        click:function(){
          ...
          let psnjobData = {};
          if (this.$parent.employeeData.psnjob) {
            for (let i = 0; i < this.$parent.employeeData.psnjob.length; i++) {
              if (this.$parent.employeeData.psnjob[i]['lastflag']) {
                psnjobData = JSON.parse(JSON.stringify(this.$parent.employeeData.psnjob[i]));
                break;
              }
            }
          }
  ...
  mounted() {
    ...
    this.employeeDetailPageInit();
  },

  methods: {
    // 对页面进行初始化
    employeeDetailPageInit: function () {
      let modifiable = this.employeeDetail.modifiable;
      if (modifiable) {
        for (let i = 0; i < modifiable.length; i++) {
          this.$set(this.employeeDetail[modifiable[i]], 'icons', this.employeeIcons[modifiable[i]]);
        }
      }
      this.getCodeEditable(this.pk_psndoc);
    },

    // 获取人员编码、员工号的可编辑性
    getCodeEditable(pk_psndoc) {
      let param = {
        pk_psndoc: pk_psndoc
      };
      this.$http({
        url: this.requestPrefix + '/getCodeEditable',
        method: "post",
        params: param
      }).then(res => {
        if (res.data.status === true) {
        ...
        this.$http({
          ...
          if (res.data.status === true) {
            ...
            if (resData) {
              ...
              if(infoSetCodes[i] === 'hi_psndoc_parttime'){
                this.partBtnShow = true;
              }
            } else {
        ...
        if (valid) {
          ...
          this.$http({
            ...
            if (res.data.status === true) {
              ...
              // 如果是修改了家庭信息，需要同步更新一下亲属规避子集
              if ('family' == codes) {
                let familyavoid = [];
                familyavoid.push('familyavoid');
                if (confirmThis.getInfoSetCodeByCodes(familyavoid).length == 1) {
                  confirmThis.dynamicTemplateInit(familyavoid);
                }
              }
    ...
    realDynamicDelete: function (data, codes) {
      let param = {
        ...
        pk_psndoc_sub: data.subInfo.pk_psndoc_sub,
      };
      this.$http({
        ...
        if (res.data.status === true) {
          ...
          if ('family' == codes) {
            let familyavoid = [];
            familyavoid.push('familyavoid');
            if (this.getInfoSetCodeByCodes(familyavoid).length == 1) {
              this.dynamicTemplateInit(familyavoid);
            }
          }
        } else {
          this.$message({message: res.data.msg, type: "error"});
    ...
    psndocEditValueChange: function (val) {
      ...
      this.employeeDetail.psndoc.edit = val;
    },

    // 人员基本信息数据改变
    psndocDataChange: function (val) {
      this.employeeData.psndoc = val;
      this.$refs[this.dynRef].employeeData = this.employeeData;
    },

    // 工作信息数据改变
    psnjobDataChange: function (val) {
      this.employeeData.psnjob = val;
      this.$refs[this.dynRef].employeeData = this.employeeData;
    },

    // 详情页面返回列表
    employeeDetailReturn() {
      this.$router.push("/hrhi/employee" + "?" + this.funcode);
    },

    // 入转调离用方法
    employeeGoToPage: function (val) {
      this.$router.push(val);
    },

    // 离职对话框
    employeeLeaveClick: function () {
    ...
    printTemplate:function () {
      ...
      this.printPre("60070employee", "employee-print-template", param);
    },
