...
<template>
  <div class="psndoc-main-avatar clearfix">
    ...
    <div class="psndoc-main-avatar-right" v-if="pk_psndoc">
      ...
      <!--<div v-if="eduName" class="psndoc-main-avatar-other" v-text="eduName"></div>-->
      ...
      <tag-dis :pk_psndoc="pk_psndoc" :pk_org="pk_org" mode="2"></tag-dis>
    </div>
  </div>
</template>
  ...
  export default {
    ...
    computed: {
      ...
      orgName: function () {
        ...
        pk_org: function () {
            if (this.psndocData.psnjob) {
                return this.psndocData.psnjob[this.psndocData.psnjob.length - 1].pk_org;
            }
            return "";
        },
    ...
    data() {
      return {
        ...
        //新增时间戳，防止缓存
        imgUrl: '/hrhi/psndoc/getPhotoByPK?pk_psndoc=' + this.pk_psndoc + '&time=' + (new Date()).getTime(),
        imgUrlNew: '/hrhi/psndoc/getPhotoByPK?pk_psndoc=' + this.pk_psndoc + '&time=' + (new Date()).getTime(),
      };
    },
