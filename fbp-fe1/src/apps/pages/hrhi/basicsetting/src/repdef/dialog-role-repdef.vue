      ...
      if (this.repdefData && this.repdefData.length > 0) {
        ...
        for (let i = 0; i < this.repdefData.length; i++) {
          if (i + 1 < this.repdefData.length && this.repdefData[i].pk_org !== this.repdefData[i + 1].pk_org) {
            this.$message({ message: '批量操作只允许选择同一个花名册的数据', type: 'error' });
            return false;
          }
          this.roleids.push(this.repdefData[i].pk_role);
        }
        if (this.roleids.length > 1) {
          this.pk_dialog = '';
        } else {
          this.pk_dialog = this.repdefData[0].pk_role;
        }
      } else {
        this.$message({ message: '请先选择角色或花名册！', type: 'error' });
        return false;
      }
      return true;
    },
