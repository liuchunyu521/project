...
export default {
    ...
    methods: {
        ...
        formEdit(type, row) {
            ...
            let pk_contedu_b = row.pk_contedu_b;
            this.conteduParam(pk_contedu_b);

        },
        formDelete(type, row) {
            let index = this.subList.indexOf(row);
            let pk_contedu_b = row.pk_contedu_b;
            this.delete(pk_contedu_b, index, row);

        },
        delete(pk_contedu_b, index, delData) {
            ...
            this.$confirm('确认删除该数据？', '提示', {
                ...
                if (pk_contedu_b) {//这一行的值已经做持久化了，最后要真的到数据库中删除
                    delData.status = 3;
        ...
        tableDeleteClick(row, rowIndex) {
            let pk_contedu_b = row.row.pk_contedu_b;
            let index = row.$index;
            let delData = row.row;
            this.delete(pk_contedu_b, index, delData);
        },
        tableEditClick(row, rowIndex) {
            ...
            let pk_contedu_b = row.row.pk_contedu_b;
            this.setwaGrdParam(pk_contedu_b);
        ...
        setTableData(bodys) {
            this.$refs.ref_nationduty_c.setTableData(JSON.parse(JSON.stringify(bodys)));
