			...
			Init:function(){
				$.ajax({
			        ...
			        success: function (res) {
			        	if(res.status==true){
			        		$("#homePage_DAP_partyMass_num").html(res.data.numberData)
			        	}else{
			        		alert('员工入职转档数量获取异常')
			        	}
				...
				var res=getUrlParam();
				$("#homePage_DAP_partyMass_box").on("click",function(){//员工入职
					//window.parent.location.hash='/hrhi/register?funcode=60070register&tilecode='+res;
				});
//				$(".homePage_DAP_partyMass_middle").on("click",function(){//员工转档
